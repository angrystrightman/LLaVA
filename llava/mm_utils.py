...
def tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    ...
    #special token 的位置 replace with the setting sepcial token num.
...
def tokenizer_audio_token(prompt, tokenizer, audio_token_index=AUDIO_TOKEN_INDEX, return_tensors=None):
    ...
    # Split the prompt by the audio token and tokenize each chunk
    ...
    # Handle the beginning of the prompt if it starts with a BOS token
    if len(prompt_chunks) > 0 and len(prompt_chunks[0]) > 0 and prompt_chunks[0][0] == tokenizer.bos_token_id:
        offset = 1
        input_ids.append(prompt_chunks[0][0])

    # Insert the audio token at the appropriate positions
    for x in insert_separator(prompt_chunks, [audio_token_index] * (offset + 1)):
        input_ids.extend(x[offset:])

    # Return the result as a tensor if requested
    if return_tensors is not None:
        if return_tensors == 'pt':
            return torch.tensor(input_ids, dtype=torch.long)
        raise ValueError(f'Unsupported tensor type: {return_tensors}')
